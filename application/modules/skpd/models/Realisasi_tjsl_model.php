...
class Realisasi_tjsl_model extends MY_Model
    {

        protected $table_name        = 'realisasi_tjsl';
        protected $key               = 'realisasi_tjsl_id';
        ...
        public function get_all_realisasi_tjsl()
        {

            $data =$this->db->select('a.*,a.email as email_pemohon,a.status as status_permohonan, b.*,c.*,d.*,e.*,f.*,g.*,h.*')
                        ...
                        ->join('realisasi_tjsl h', 'a.permohonan_tjsl_id = h.permohonan_tjsl_id')
                        ->where('h.deleted',0)
                        ->where('a.status',1)
                        ->get()
                        ->result();
            ...
            foreach ($data as $row) {
                ...
                if($row->status_realisasi == 0){
                  $status_realisasi= "<a href='#' class='btn waves-effect waves-light btn-xs btn-danger' disabled> <i class='fa fa-watch'> </i> Belum Realisasi</a>";
                  $realisasi = "<button type='button' class='realisasi btn waves-effect waves-light btn-xs btn-success' data-id='$row->permohonan_tjsl_id'><i class='fa fa-check-square'></i> Realisasikan </button> ";
                 } else{
                  $status_realisasi = "<a href='#'' class='btn waves-effect waves-light btn-xs btn-primary' disabled><i class='fa fa-check-square'> </i> Sudah Realisasi</a>";
                  $batal = "<button type='button' class='batal btn waves-effect waves-light btn-xs btn-danger' data-id='$row->permohonan_tjsl_id'><i class='fa fa-close'></i> Batal </button> ";
                 }
                ...
                $array[] = 'Rp. ' .number_format($row->nilai_rab,0,',','.');
                 ...
                 if($row->status_realisasi == 0){
                    $array[] = $status_realisasi;
                 }else{
                    $array[] = $status_realisasi;
                 }
                $array[] = 'Rp. ' .number_format($row->nominal,0,',','.');
        ...
        public function get_realisasi_tjsl($id = '')
        {
            // print_r($id);die();
        ...
        public function update_tbl_realisasi($id,$data,$table){
        
        $this->db->where('permohonan_tjsl_id',$id);
